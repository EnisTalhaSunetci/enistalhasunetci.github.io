...
export default function AdminPage() {
  ...
  const TABS = [
    ...
    { key: "github", label: "GitHub Senkronizasyonu" },
  ];
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-8">
          ...
          <div className="card p-6 md:p-8">
            ...
            { activeTab === "github" && <GithubSyncPanel data={data} />}
          </div>
        </div>
      </div>
    </div>
...
function GithubSyncPanel({ data }: { data: SiteData }) {
  ...
  const handleSync = async () => {
    if (!token) {
      alert("Lütfen GitHub Personal Access Token (PAT) girin.");
      return;
    }
    setIsSyncing(true);
    setStatus("GitHub deposu sorgulanıyor...");
    try {
      const repo = "EnisTalhaSunetci/enistalhasunetci.github.io";
      const path = "lib/defaultData.ts";

      // 1. Get the current file content to retrieve the SHA
      ...
      if (!getRes.ok) {
        throw new Error("GitHub bağlantısı başarısız. Lütfen Token ayarlarınızı kontrol edin.");
      }

      const fileData = await getRes.json();
      const sha = fileData.sha;

      setStatus("Değişiklikler şifreleniyor ve gönderiliyor...");

      // 2. Generate the full TypeScript file content
...
export const DEFAULT_SITE_DATA: SiteData = ${JSON.stringify(data, null, 2)};
`;

      const utf8Bytes = new TextEncoder().encode(newFileContent);
      const base64Content = btoa(String.fromCharCode(...utf8Bytes));

      // 3. Make PUT request to GitHub to update the file
      ...
      if (putRes.ok) {
        ...
        setStatus("Senkronizasyon Başarılı! Değişiklikler GitHub sunucusuna yüklendi. Siteniz 1-2 dakika içinde güncellenecektir.");
      } else {
        const errJson = await putRes.json();
        throw new Error(errJson.message || "Dosya güncellenemedi.");
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-bold text-white mb-2">GitHub Otomatik Senkronizasyonu</h2>
      <p className="text-sm text-[#9090A8] mb-6">
        Admin panelinden yaptığınız değişikliklerin telefonunuzda ve tüm ziyaretçilerde anında geçerli olması için sitenizi GitHub sunucusunda otomatik olarak güncelleyebilirsiniz.
      </p>

      <div className="p-4 rounded-xl bg-purple-500/10 border border-purple-500/20 text-xs text-[#A78BFA] leading-relaxed mb-6">
        <strong>Nasıl Kullanılır?</strong>
        <ol className="list-decimal list-inside mt-2 space-y-1">
          <li>GitHub hesabınızdan bir <strong>Personal Access Token (PAT)</strong> oluşturun (repo yetkisiyle).</li>
          <li>Aşağıdaki alana bu Token değerini yapıştırın.</li>
          <li><strong>"GitHub Sunucusuna Kaydet"</strong> butonuna basın.</li>
          <li>Sistem arka planda dosyayı güncelleyecek ve GitHub otomatik olarak sitenizi yeniden derleyecektir.</li>
        </ol>
      </div>

      <div className="space-y-4">
        <div>
          <label className="block text-xs text-[#4A4A60] font-semibold uppercase tracking-wider mb-2">GitHub Personal Access Token (PAT)</label>
          <input 
            type="password" 
            value={token} 
            onChange={(e) => {
        ...
        <button 
          onClick={handleSync} 
          disabled={isSyncing}
          className={`px-6 py-2.5 rounded-xl text-white text-sm font-medium transition-colors w-full ${isSyncing ? 'bg-purple-600/50 cursor-not-allowed' : 'bg-[#7C3AED] hover:bg-[#6D28D9]'}`}
        >
